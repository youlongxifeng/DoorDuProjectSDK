...
//					if (statusCode != 0)
//						throw new VolleyError(new NetworkResponse(statusCode, null, null, false));
//					else
                ...
                RecordBuilder.http(0,"Unexpected response code %d for %s", statusCode, request.getUrl());
//				VolleyLog.e("Unexpected response code %d for %s", statusCode, request.getUrl());
				if (responseContents != null) {
					networkResponse = new NetworkResponse(statusCode, responseContents, responseHeaders, false);
					if (  statusCode == HttpStatus.SC_UNAUTHORIZED || statusCode == HttpStatus.SC_FORBIDDEN) {
						attemptRetryOnException("auth", request, new AuthFailureError(networkResponse));
					} else if(statusCode == HttpStatus.SC_INTERNAL_SERVER_ERROR || statusCode == HttpStatus.SC_REQUEST_TIMEOUT|| statusCode == HttpStatus.SC_SERVICE_UNAVAILABLE ||  statusCode == HttpStatus.SC_NOT_FOUND ) {
					    attemptRetryOnException(String.format("errorcode-%d", statusCode), request, new VolleyError(new NetworkResponse(statusCode, null, null, false)));
					}else{
	...
	private static void attemptRetryOnException(String logPrefix, Request<?> request, VolleyError exception)
        ...
        RecordBuilder.http(0,String.format("%s-retry [timeout=%s]", logPrefix, oldTimeout));
