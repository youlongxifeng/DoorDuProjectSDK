...
import com.android.volley.toolbox.Secret;
...
public abstract class Request<T> implements Comparable<Request<T>> {
	...
	private final MarkerLog mEventLog = VolleyLog.DEBUG ? new MarkerLog() : null;
	 ...
	 */
	private long mCacheLive;
	/**
	 * 
	 * 缓存的刷新间隔
	 */
	private long mRefershInterval;
	...
	public void setRefershInterval(long millSeconds) {
		mRefershInterval = millSeconds;
	}

	protected long getCacheLive() {
		return mCacheLive;
	}

	public void setCacheLive(long millSeconds) {
		mCacheLive = millSeconds;
	}

	protected long getRefershInerval() {
		return mRefershInterval;
	}
	 ...
	 * 
	 * @deprecated Use
	 *             {@link #Request(int, String, ErrorListener)}
	 *             .
	 */
	...
	public Request(int method, String url, ErrorListener listener) {
		...
		if (!TextUtils.isEmpty(url)) {
			...
			mDefaultTrafficStatsTag = TextUtils.isEmpty(s) ? 0 : s.hashCode();
		} else {
	...
	void finish(final String tag) {
		...
		if (null != mEventLog) {
			...
			mEventLog.add(tag, threadId);
			if(null != mProxy)
			 mEventLog.add("proxy address "+ mProxy.toString(), threadId);
			mEventLog.finish(this.toString());
		} else {
	...
	public void deliverError(VolleyError error) {
		if (mErrorListener != null) {
			mErrorListener.onErrorResponse(error, mToken);
		}
	}
	...
	public byte[] decryption(byte[] data, int start, int count) {
		if (null != mSecret && null != data)
			return mSecret.decryption(data, start, count);
		return data;
	}

	public byte[] encryption(byte[] data, int start, int count) {
		if (null != mSecret && null != data)
			return mSecret.encryption(data, start, count == 0 ? data.length : count);
		return data;
	}
